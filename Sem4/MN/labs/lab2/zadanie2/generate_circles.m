function [circles, a, b, r_max] = generate_circles(n_max)
    ...
    circle_areas = zeros(n_max, 1); % inicjalizacja wektoru kolumnowego dla kumulowania sumy pól
    area_sum = 0;
    ...
    while count < n_max
        ...
        if valid
            count = count + 1;
            circles(count, :) = [X, Y, R];
            area_sum = area_sum + pi * R^2; % Dodajemy pole do sumy pól
            circle_areas(count, 1) = (area_sum / (a * b)) * 100; 
        end
    end

    plot(circle_areas)
    title("Cumulative sum of circle areas")
    xlabel("Number of circles")
    ylabel("Procent of filling")
end
